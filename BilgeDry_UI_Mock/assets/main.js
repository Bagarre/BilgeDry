...
window.addEventListener('load', async () => {
  ...
  async function hitSim(path, okMsg) {
    try {
      ...
      console.error(err);
      showToast('Simulator action failed', true);
    }
  }

  document.getElementById('simulateWetBtn')?.addEventListener('click', async () => hitSim('/api/v1/sim/wet', 'Wet scenario triggered'));
  document.getElementById('simulateFaultBtn')?.addEventListener('click', async () => hitSim('/api/v1/sim/fault', 'Fault scenario triggered'));
  document.getElementById('simulateIdleBtn')?.addEventListener('click', async () => hitSim('/api/v1/sim/idle', 'Returned to idle'));
});
